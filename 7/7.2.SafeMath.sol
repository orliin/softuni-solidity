...
contract OwnedSafeMath is Owned, SafeMath {
    
    int256 public state;
    uint256 public lastChange = 1;
    
    function change() onlyOwner public {
        state = add(state, int256(now) % 256);
        state = multiply(state, subtract(int256(now),int256(lastChange)));
        state = subtract(state, int256(block.gaslimit));
